...
/*   Updated: 2024/05/04 20:21:42 by lsampiet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void validate_collects(t_game *game)
{
	...
	if (game->map[game->player_pos.y][game->player_pos.x] == 'C' && game->occ.collects > 0)
	{
		...
		{
			if (game->image->collects_img->instances[i].x == game->image->player_img->instances[0].x && game->image->collects_img->instances[i].y == game->image->player_img->instances[0].y)
